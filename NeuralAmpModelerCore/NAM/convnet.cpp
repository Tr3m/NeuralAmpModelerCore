...
nam::convnet::ConvNet<SampleType>::ConvNet(const int channels, const std::vector<int>& dilations, const bool batchnorm,
                               const std::string activation, std::vector<float>& weights,
                               const double expected_sample_rate)
: Buffer<SampleType>(*std::max_element(dilations.begin(), dilations.end()), expected_sample_rate)
{
  ...
  for (size_t i = 0; i < dilations.size(); i++)
    this->_prewarm_samples += dilations[i];
}

template <typename SampleType>
void nam::convnet::ConvNet<SampleType>::process(SampleType* input, SampleType* output, const int num_frames)
...
template <typename SampleType>
void nam::convnet::ConvNet<SampleType>::_verify_weights(const int channels, const std::vector<int>& dilations, const bool batchnorm,
...
template <typename SampleType>
void nam::convnet::ConvNet<SampleType>::_update_buffers_(SampleType* input, const int num_frames)
{
  this->Buffer<SampleType>::_update_buffers_(input, num_frames);
...
template <typename SampleType>
void nam::convnet::ConvNet<SampleType>::_rewind_buffers_()
{
...
template class nam::convnet::ConvNet<float>;
template class nam::convnet::ConvNet<double>;
...
public:
  ...
  DSP(const SampleType expected_sample_rate);
  virtual ~DSP() = default;
  ...
  virtual void process(SampleType* input, SampleType* output, const int num_frames);
  ...
  void SetLoudness(const SampleType loudness);

protected:
  bool mHasLoudness = false;
  // How loud is the model? In dB
  SampleType mLoudness = 0.0;
  // What sample rate does the model expect?
  SampleType mExpectedSampleRate;
...
{
public:
  Buffer(const int receptive_field, const SampleType expected_sample_rate = -1.0);
...
protected:
  ...
  virtual void _update_buffers_(SampleType* input, int num_frames);
...
public:
  Linear(const int receptive_field, const bool _bias, const std::vector<float>& weights,
         const SampleType expected_sample_rate = -1.0);
  void process(SampleType* input, SampleType* output, const int num_frames) override;
...
struct dspData
{
  ...
  SampleType expected_sample_rate;
};
...
template<typename SampleType>
std::unique_ptr<DSP<SampleType>> get_dsp(const std::filesystem::path model_file, dspData<SampleType>& returnedConfig);

...
std::vector<SampleType> GetWeights(nlohmann::json const& j, const std::filesystem::path config_path)
{
...
{
  dspData<SampleType> temp;
  return get_dsp<SampleType>(config_filename, temp);
}

template <typename SampleType>
std::unique_ptr<DSP<SampleType>> get_dsp(const std::filesystem::path config_filename, dspData<SampleType>& returnedConfig)
{
  ...
  std::vector<float> weights = GetWeights<SampleType>(j, config_filename);
  ...
  return get_dsp<SampleType>(conf);
}

template <typename SampleType>
std::unique_ptr<DSP<SampleType>> get_dsp(dspData<SampleType>& conf)
{
  ...
  std::unique_ptr<DSP<SampleType>> out = nullptr;
  if (architecture == "Linear")
  {
    const int receptive_field = config["receptive_field"];
    const bool _bias = config["bias"];
    out = std::make_unique<Linear<SampleType>>(receptive_field, _bias, weights, expectedSampleRate);
  }
  else if (architecture == "ConvNet")
  {
    ...
    out = std::make_unique<convnet::ConvNet<SampleType>>(channels, dilations, batchnorm, activation, weights, expectedSampleRate);
  }
  else if (architecture == "LSTM")
  {
    ...
    out = std::make_unique<lstm::LSTM<SampleType>>(num_layers, input_size, hidden_size, weights, expectedSampleRate);
  }
  else if (architecture == "WaveNet")
  {
    ...
    out = std::make_unique<wavenet::WaveNet<SampleType>>(layer_array_params, head_scale, with_head, weights, expectedSampleRate);
  }
  else
  {
...
void dummy()
{
  auto oneFloat = get_dsp<float>("wtf");
  auto oneDouble = get_dsp<float>("wtf");
}
}; // namespace nam

...
namdsp::noise_gate::Trigger<SampleType>::Trigger()
...
template <typename SampleType>
SampleType** namdsp::noise_gate::Trigger<SampleType>::Process(SampleType** inputs, const size_t numChannels, const size_t numFrames)
{
  ...
  for (auto c = 0; c < numChannels; c++)
  {
    for (auto s = 0; s < numFrames; s++)
    {
      ...
      if (this->mState[c] == namdsp::noise_gate::Trigger<SampleType>::State::HOLDING)
      {
        ...
        if (levelDB < threshold)
        {
          ...
          if (this->mTimeHeld[c] >= maxHold)
            this->mState[c] = namdsp::noise_gate::Trigger<SampleType>::State::MOVING;
        }
        else
        {
  ...
  for (auto c = 0; c < numChannels; c++)
    memcpy(this->mOutputs[c].data(), inputs[c], numFrames * sizeof(SampleType));
  return this->_GetPointers();
}

template <typename SampleType>
void namdsp::noise_gate::Trigger<SampleType>::_PrepareBuffers(const size_t numChannels, const size_t numFrames)
{
  ...
  if (updateChannels || updateFrames)
  {
    ...
    if (updateChannels)
    {
      ...
      std::fill(this->mState.begin(), this->mState.end(), namdsp::noise_gate::Trigger<SampleType>::State::MOVING);
...
template <typename SampleType>
SampleType** namdsp::noise_gate::Gain<SampleType>::Process(SampleType** inputs, const size_t numChannels, const size_t numFrames)
{
...
template class namdsp::noise_gate::Gain<float>;
template class namdsp::noise_gate::Gain<double>;
template class namdsp::noise_gate::Trigger<float>;
template class namdsp::noise_gate::Trigger<double>;
template class namdsp::noise_gate::TriggerParams<float>;
template class namdsp::noise_gate::TriggerParams<double>;